...
fb::Result setTyping(ID chatID, bool wait = true);

// установить заголовок чата
fb::Result setChatTitle(ID chatID, Text title, bool wait = true);

// установить описание чата
fb::Result setChatDescription(ID chatID, Text description, bool wait = true);
...
// закрепить сообщение
fb::Result pinChatMessage(ID chatID, ID messageID, bool notify = true, bool wait = true);

// открепить сообщение
fb::Result unpinChatMessage(ID chatID, ID messageID, bool wait = true);

// открепить все сообщения
fb::Result unpinAllChatMessages(ID chatID, bool wait = true);
...
// удалить сообщение
fb::Result deleteMessage(ID chatID, ID messageID, bool wait = true);

// удалить сообщения
fb::Result deleteMessages(ID chatID, uint32_t* messageIDs, uint16_t amount, bool wait = true);
...
```cpp
Message();
Message(const String& text, ID chatID);
...
```cpp
MessageForward();
MessageForward(uint32_t messageID, ID fromChatID, ID chatID);
...
<details>
<summary>fb::InlineKeyboard</summary>

```cpp
InlineKeyboard();
InlineKeyboard(const String& text, const String& data);
InlineKeyboard(uint16_t reserve);
...
```cpp
Location();
Location(float latitude, float longitude, ID chatID);
...
```cpp
TextEdit();
TextEdit(const String& text, uint32_t messageID, ID chatID);
...
<details>
<summary>fb::KeyboardEdit</summary>

```cpp
KeyboardEdit();
KeyboardEdit(uint32_t messageID, ID chatID);
KeyboardEdit(uint32_t messageID, ID chatID, InlineKeyboard* menu);
...
```cpp
CaptionEdit();
CaptionEdit(const String& caption, uint32_t messageID, ID chatID);
...
```cpp
LocationEdit();
LocationEdit(float latitude, float longitude, uint32_t messageID, ID chatID);
...
```cpp
LocationStop();
LocationStop(uint32_t messageID, ID chatID);
