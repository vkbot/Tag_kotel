...
extern void sendMsg(String text, String chatId);
...
bool sendRelayCommand(bool on) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (shouldSendError()) serialToTelegram("❌ Wi-Fi отключён.");
    return false;
  }
  ...
  // Успех: любой 2xx или 3xx
  if (httpCode >= 200 && httpCode < 400) {
    http.end();
    failCount = 0;
    if (!relayFound) {
      relayFound = true;
      serialToTelegram("✅ Реле доступно: " + relayIP.toString());
    }
    return true;
  }

  // Реле перезагрузилось или разорвало соединение после выполнения
  if (httpCode == HTTPC_ERROR_NO_HTTP_SERVER || httpCode == HTTPC_ERROR_CONNECTION_LOST) {
    Serial.println("⚠️ Реле сработало, но не ответило");
    http.end();
    failCount = 0;
    return true;
  }
  ...
  if (httpCode == HTTPC_ERROR_CONNECTION_REFUSED || 
    ...
    failCount++;
    if (shouldSendError()) {
      serialToTelegram("🚨 Ошибка реле: " + String(httpCode));
    }
    if (failCount >= 3) {
      relayFound = false;
      serialToTelegram("⛔ Реле не отвечает 3 раза");
      failCount = 0;
    }
    http.end();
    return false;
  ...
  // Всё остальное — считаем, что команда ушла
  Serial.println("⚠️ Код ответа: " + String(httpCode) + ", считаем ОК");
  http.end();
  failCount = 0;
  return true;
  ...
  if (!thermostatEnabled) {
    if (relayIsRunning || relayState) {
      ...
      if (success) {
        ...
        sendMsg("🛑 Термостат выключен — реле ОТКЛЮЧЕНО",CHAT_ID);
      }
    }
    return;
  }
...
void boilerLoop() {
  ...
  if (workScheduled && workModeEnabled) {
    ...
    if (intervalOver) {
        ...
        sendMsg("✅ Временной интервал работы окончен. Переключаемся в АВТО",CHAT_ID);
    }
}
