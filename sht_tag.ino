...
const char* CHAT_ID   = "619084238";  // Дефолтный чат
...
bool sendTG(const String& msg, const String& chatId = "") {
    ...
    if (!tgClient.connect(WORKER, 443)) {
        tgClient.stop(); delay(200);
        if (!tgClient.connect(WORKER, 443)) return false;
    }

    // 🔥 ПЕРЕХОДИМ НА POST (надёжнее для CF и Telegram API)
    ...
    tgClient.print("User-Agent: ESP8266Bot/1.0\r\n");
    ...
    while (!tgClient.available() && millis() - waitStart < 3000) {
        delay(10); yield();
    }
    ...
    while (tgClient.connected() || tgClient.available()) {
        if (tgClient.available()) {
            ...
            response += c;
            lastByteTime = millis();
            if (response.length() > 500) break; // Защита от переполнения
        }
        if (!tgClient.connected() && millis() - lastByteTime > 800) break;
        delay(10); yield();
    }
    tgClient.stop();
    delay(150);
    ...
    if (!success) {
        Serial.print(">>> sendTG FAILED: ");
        int len = response.length();
        if (len > 200) len = 200;
        Serial.println(response.substring(0, len));
    }
    return success;
}
...
void loadLastUpdateID() {
    ...
    // Валидные update_id в Telegram всегда >= 0, но 0 — это старт.
    // Если читаем "мусор" (отрицательное число или слишком большое), сбрасываем.
    if (tmp < 0) {
        Serial.println(F("⚠️ EEPROM: garbage detected, reset offset"));
        lastUpdateID = 0;
    } else {
        lastUpdateID = tmp;
        Serial.print(F("✅ EEPROM: loaded offset "));
...
{  
 ...
 bot.setBufferSizes(1024, 1024);
    ...
    loadLastUpdateID();
    // ▼▼▼ ОТЛАДКА OFFSET ▼▼▼
Serial.print(">>> DEBUG: loaded offset = ");
Serial.println(lastUpdateID);

Serial.println(">>> DEBUG: calling bot.setOffset()...");
bot.setOffset(lastUpdateID);  // Если библиотека не патчена — будет ошибка компиляции здесь!
Serial.println(">>> DEBUG: setOffset() called OK");
// ▲▲▲ КОНЕЦ ОТЛАДКИ ▲▲▲
    bot.setOffset(lastUpdateID);
    ...
    bot.attach([](FB_msg& msg)
{
   ...
   if (msg.chatID.toInt() == lastUserChatID && millis() - lastUserCommandTime < COMMAND_COOLDOWN_MS) {
    ...
    // ▼▼▼ ДОБАВЬ ЭТУ СТРОКУ ▼▼▼
    Serial.print(">>> CALLBACK: saving offset "); 
    Serial.println(msg.update_id + 1); // Показываем, что ХОТИМ сохранить
    // ▲▲▲ КОНЕЦ ▲▲▲
    ...
    else if (cmd == "/help")
{
    ...
    String helpMsg1 = "🆘 *Основные команды*:\n";
    helpMsg1 += "/status — статус, аптайм, датчики\n";
    helpMsg1 += "/autoon, /autooff — автоотчёт вкл/выкл\n";
    helpMsg1 += "/setinterval N — интервал (30-86400 сек)\n";
    helpMsg1 += "/setwifi SSID PASS — сменить сеть\n";
    helpMsg1 += "/reboot — перезагрузка\n";
    helpMsg1 += "/help — эта справка";
    sendMsg(helpMsg1, cid);
    delay(1000); // Пауза между сообщениями
    bot.tick();       // ← Прогнать очередь
    // Часть 2: Термостат
    String helpMsg2 = "\n🔥 *Термостат*:\n";
    helpMsg2 += "/thermo on/off — вкл/выкл термостат\n";
    helpMsg2 += "/addzone ЧЧ:ММ-ЧЧ:ММ ТЕМП — зона (7:00-22:00 22.5)\n";
    helpMsg2 += "/delzone N — удалить зону\n";
    helpMsg2 += "/zones — показать расписание\n";
    helpMsg2 += "/sethyst 0.5 — гистерезис (0.1-2.0)\n";
    helpMsg2 += "/setworktime N — макс. работа реле (1-60 мин)\n";
    helpMsg2 += "/setminpause N — мин. пауза (1-60 мин)\n";
    helpMsg2 += "/clearzones — очистить все зоны";
    sendMsg(helpMsg2, cid);
}
else if (cmd == "/forceon") {
